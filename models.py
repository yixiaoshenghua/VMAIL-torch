...
class ImagBehavior(nn.Module):
    ...
    def __init__(self, config, world_model, stop_grad_actor=True, reward=None):
        ...
        self.value = networks.DenseHead(
                ...
                config.value_head)
        self.discriminator = networks.DenseHead(
                feat_size+config.num_actions, [], 2, config.units, config.act, 'binary')
        ...
        self._discriminator_opt = tools.Optimizer(
                'discriminator', self.discriminator.parameters(), config.discriminator_lr, config.opt_eps, config.grad_clip, 
                **kw)

    def _train(
            self, start, expert_data, objective=None, action=None, reward=None, imagine=None, tape=None, repeats=None):
        ...
        with tools.RequiresGrad(self.discriminator):
            with torch.cuda.amp.autocast(self._use_amp):
                with torch.no_grad():
                    ...
                    post_expert, prior_expert = self._world_model.dynamics.observe(embed_expert, expert_data['action'])
                    post_expert = {k: v.detach() for k, v in post_expert.items()}
                    feat_expert = self._world_model.dynamics.get_feat(post_expert)
                feat_expert_dist = torch.cat([feat_expert, expert_data['action']], dim=-1)
                feat_policy_dist = torch.cat([imag_feat, imag_action], dim=-1)
                expert_d = self.discriminator(feat_expert_dist)
                policy_d = self.discriminator(feat_policy_dist)
                expert_loss = torch.mean(expert_d.log_prob(torch.ones_like(expert_d.mode()).to(self._config.device)))
                policy_loss = torch.mean(policy_d.log_prob(torch.zeros_like(policy_d.mode()).to(self._config.device)))
                # gradient penalty
                alpha = torch.rand(feat_policy_dist.shape[0], feat_policy_dist.shape[1]).to(self._config.device).unsqueeze(-1)
                disc_penalty_input = alpha * feat_policy_dist + (1 - alpha) * feat_expert_dist.reshape((-1, feat_policy_dist.shape[-1])).unsqueeze(0).tile([self._config.imag_horizon, 1, 1])
                disc_penalty_input.requires_grad_(True)
                disc_penalty_output = self.discriminator(disc_penalty_input).mode()
                grad_outputs = torch.ones_like(disc_penalty_output).to(self._config.device)
                gradients = torch.autograd.grad(
                    outputs = disc_penalty_output,
                    inputs = disc_penalty_input,
                    grad_outputs = grad_outputs,
                    create_graph = True,
                    retain_graph = True
                )[0]
                gradients = gradients.view(-1, feat_expert_dist.shape[-1])
                grad_norm = gradients.norm(2, 1)
                grad_penalty = torch.mean((grad_norm - 1))**2

                discriminator_loss = -(expert_loss + policy_loss) + self._config.alpha * grad_penalty   
        ...
        metrics['expert_loss'] = to_np(expert_loss)
        metrics['policy_loss'] = to_np(policy_loss)
        metrics['grad_penalty'] = to_np(grad_penalty)
